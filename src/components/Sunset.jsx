...
const Sunset = ({ infoToPass }) => {

  const [currentTimestamp, setCurrentTimestamp] = useState('');
  const [sunsetTimeStamp, setSunsetTimestamp] = useState('');

  let sunsetDateObj = new Date(sunsetTimeStamp);

  useEffect(() => {
    setCurrentTimestamp(infoToPass.dt);
    setSunsetTimestamp(infoToPass.sunset);
  })
  ...
  return (
    <div className="d-flex justify-content-between condition-tab">
      <div className="d-flex internal-icon">
        ...
        <div>
          <h5>Sunset</h5>
          <p className="condition-detail">{(infoToPass === undefined) ? null : infoToPass.sunset}</p>
        </div>
      </div>
